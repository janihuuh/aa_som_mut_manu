...
getVDJforFile <- function(file_temp){
  ...
  get_vdj <- function(file_temp, clonotype_id, chain){
    ...
    conv_temp <- conv_temp %>%
      mutate(cdr3s_nt = as.factor(cdr3s_nt), cdr3s_aa = as.factor(cdr3s_aa), clonotype_id = as.factor(clonotype_id)) %>%
      mutate(cdr3s_nt = droplevels(cdr3s_nt), cdr3s_aa = droplevels(cdr3s_aa), clonotype_id = droplevels(clonotype_id))
  ...
  file_temp$clonotype_id <- as.factor(file_temp$clonotype_id)
...
newClonotype_df <- function(old_clonotypes_df){
  ...
  for(cdr3 in levels(old_clonotypes_df$cdr3s_nt)){
